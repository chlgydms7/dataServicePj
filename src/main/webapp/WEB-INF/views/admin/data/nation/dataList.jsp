                                ...
                                <div class="row align-items-center">
                                    ...
                                    <div class="col-md-4">
                                        <ul class="breadcrumb">
                                            ...
                                            <li class="breadcrumb-item"><a href="/nationList">국가공간정보데이터</a>
                                            </li>
                                        </ul>
                                    </div>
                                </div>
                            ...
                            <form name="ManageVO" method="POST">
                                <div class="page-wrapper">
                                        ...
                                        <div class="card">
                                            <div class="card-block table-border-style">
                                                <div class="table-responsive">
	 												...
	 												<input type="hidden" name="metdataId" id="metdataId" value="" />
                                                        ...
                                                        <tbody>
                                                            <c:if test="${totalCount != 0}">
																<c:forEach items="${lists }" var="dataList">
																	<tr>
																		<td> ${dataList.rn }</td>
																		<td onclick="nationDetail('${dataList.metdataId}');" style="cursor: pointer;">${dataList.dataNm }</td>
																		<td>${dataList.dataTyCode }</td>
																		<%-- <td><fmt:parseDate pattern="yyyy-MM-dd HH:mm:ss" value="${dataList.regDt }"/></td> --%>
				                    ...
				                    <div style="padding-left: 90%;">
										<button type="button" onclick="insertNationData();" class="btn btn-inverse waves-effect waves-light">등록</button>
									</div>
								</div>
							</form>
                                    <!-- Page-body end -->
...
<!-- Hover table card end -->
	<!-- container Start-->
	 <%-- <form name="ManageVO" method="POST">
	 	<input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}" />
		<div class="container"	style="margin-top: 100px; width: 100%; height: 100%">
		...
		<input type="hidden" name="metdataId" id="metdataId" value="" />
				...
				<div class="single_sidebar_widget search_widget" style="width: 1200px; background:none; margin-top: -243px;">
						<div class="form-group">
							<div class="input-group mb-3">
								<!-- <label for="searchCondition" class="scondition_label"><b>검색조건</b></label> -->
								<select class="select" name="searchCondition" id="searchCondition" style="line-height: 49px; height: 51px !important;">
									<option value="dataNm" <c:if test="${vo.searchCondition eq 'dataNm' }">selected="selected"</c:if>>데이터 명</option>
									<option value="dataTyCode" <c:if test="${vo.searchCondition eq 'dataTyCode' }">selected="selected"</c:if>>데이터 유형</option>
								</select>
								<input type="text" class="form-control" id="searchKeyword" name="searchKeyword" style="margin-left: 5px;" placeholder='검색어를 입력하세요.'	onfocus="this.placeholder = ''" onblur="this.placeholder = '검색어를 입력하세요.'" value="${vo.searchKeyword }">
								<div class="input-group-append">
									<button class="btns" type="button" id="searchBtn" onclick="search();">
										<i class="ti-search"></i>
									</button>
								</div>
							</div>
			...
			<div style="margin-bottom: 50px;">
				<button type="button" onclick="insertNationData();" class="border-btn" style="float: right;">등록</button>
			</div>
		</div>
	</form> --%>
	<!-- container end -->
